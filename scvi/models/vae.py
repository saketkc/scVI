...
from scvi.models.modules import LinearDecoderSCVIGene
from scvi.models.modules import DecoderSCVIGene
from scvi.models.modules import DecoderSCVIGene2
...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        adjust_cell_counts: bool = False,
        adjust_gene_counts: bool = False
    ):
        super().__init__()
        ...
        self.adjust_cell_counts = adjust_cell_counts
        self.adjust_gene_counts = adjust_gene_counts
    ...
    def forward(
        ...
        mean_g = torch.mean(x, dim=0)
        ###print("Mean g {}".format(mean_g.shape))
        ###print("px_rate {}".format(px_rate.shape))
        ###print("px_r {}".format(px_r.shape))

        reconst_loss = self.get_reconstruction_loss(x, px_rate, px_r, px_dropout, mean_g)
...
class LDVAECENTERED(LDVAE):
    ...
    def forward(
        ...
        # KL Divergence
        mean = torch.zeros_like(qz_m) - torch.mean(x, dim=1)
        scale = torch.ones_like(qz_v)
...
class VAECell(nn.Module):
    ...
    def __init__(
        ...
        self.n_labels = n_labels
        self.latent_distribution = latent_distribution
        #self.log_umi = torch.zeros(n_input)
        if self.dispersion == "gene":
            self.px_r = torch.nn.Parameter(torch.randn(n_input))
        elif self.dispersion == "gene-batch":
            self.px_r = torch.nn.Parameter(torch.randn(n_input, n_batch))
    ...
    def get_reconstruction_loss(
        ...
        adjusted_mu = torch.exp(torch.log(px_rate+1) - torch.log(cell_mean+1))
        if self.reconstruction_loss == "zinb":
            reconst_loss = (
                -ZeroInflatedNegativeBinomial(
                    mu=adjusted_mu, theta=px_r, zi_logits=px_dropout
                )
                .log_prob(x)
                .sum(dim=-1)
            )
            ...
            reconst_loss = (
                -NegativeBinomial(mu=adjusted_mu, theta=px_r).log_prob(x).sum(dim=-1)
            )
        elif self.reconstruction_loss == "poisson":
            reconst_loss = -Poisson(adjusted_mu).log_prob(x).sum(dim=-1)
        return reconst_loss
...
class LDVAEGene(VAE):
    ...
    def __init__(
        ...
        self.decoder = LinearDecoderSCVIGene(
            n_latent,
            n_input,
            n_cat_list=[n_batch],
            use_batch_norm=use_batch_norm,
